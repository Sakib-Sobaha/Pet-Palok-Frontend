...
const MiddleLayoutChatbox = () => {
  ...
  const fetchChatHistory = async () => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/messages/${user?.id}/${selectedUser?.id}`, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  const onMessageReceived = (payload) => {
    ...
    if (message.senderId === senderId || message.senderId === recipientId) {
  ...
  return (
    <div className="flex">
      <div className="w-1.5/4 p-4 border-r border-gray-300">
            ...
            {selectedUser ? (
              <>
                <div className="bg-white p-4 border-b">
                  <h2 className="text-xl font-bold">Chat with {selectedUser.firstname} {selectedUser.lastname}</h2>
                </div>
